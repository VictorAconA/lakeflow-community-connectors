...
class LakeflowConnect:
    ...
    def _get_headers(self, folder_id: str) -> dict:
        """
    ...
    def read_table(
        ...
        For queue_items, table_options can include:
            - folder_id (required): Organization unit ID (folder) where the queue exists
        ...
        if table_name == "queue_items":
            ...
            if not folder_id:
                raise ValueError(
                    "table_options must include 'folder_id' for queue_items table"
                )
            ...
            records_iterator = self._export_queue_items(
                folder_id, queue_definition_id, filter_expr, expand
            )
    ...
    def _export_queue_items(
        self, folder_id: str, queue_definition_id: str, filter_expr: str, expand: str
    ) -> Iterator[dict]:
        """
        ...
        Args:
            folder_id: Organization unit ID (folder) where the queue exists
            queue_definition_id: ID of the queue to export
            filter_expr: OData filter expression
            expand: Related entities to expand
        """
        # Step 1: Initiate export
        export_id = self._initiate_export(folder_id, queue_definition_id, filter_expr, expand)

        # Step 2: Poll export status until completed
        self._poll_export_status(folder_id, export_id)

        # Step 3: Get download link and download CSV
        download_url = self._get_download_link(folder_id, export_id)
    ...
    def _initiate_export(
        ...
        headers = self._get_headers(folder_id)
    ...
    def _poll_export_status(
        self, folder_id: str, export_id: int, max_wait_seconds: int = 600, poll_interval: int = 5
    ) -> None:
        """
    ...
    def _get_download_link(self, folder_id: str, export_id: int) -> str:
        """
